...
export class SolutionDetail extends React.Component<SolutionDetailProp, SolutionListState> {
    ...
    constructor(prop: SolutionDetailProp) {
        ...
        this.state = {
            ...
            allItems: [],
            allRows: [],
            focusOnEntity: !prop.OnlySolutionComponent
        };
    ...
    async load() {
        ...
        //});
        console.log('Requested solution components: ' + this.state.focusOnEntity);
        debugger;
        const data =
            this.state.focusOnEntity ?
                await this.connector.GetSolutionDetailsForConnectionStrings({
                    ConnectionStrings: this.props.connectionStrings.map(e => {
                        return {
                            Id: e.Id, Url: e.OrganizationUrl, Client: e.ClientId, Secret: e.Secret
                        }
                ...
                : await this.connector.GetEntityDetailsForConnectionStrings({
                    ConnectionStrings: this.props.connectionStrings.map(e => {
                        return {
                            Id: e.Id, Url: e.OrganizationUrl, Client: e.ClientId, Secret: e.Secret
                        }
            ...
            data.forEach(e => {
                allSolutions = allSolutions.concat(e.SolutionDetails.map(a => {
                    return {
                        ...
                        connectionUrl: e.Url,
                        modifiedOn: a.ModifiedOn ? a.ModifiedOn : new Date('01/01/1900'),
    ...
    handleExport(_ev: any) {
        ...
        const colNames = this.props.connectionStrings.map(e => `${e.OrganizationUrl.replace(".com", "")}.com`);
        const firstRow: { key: string, label: string }[] = this.state.columns.filter(e => !colNames.includes(e.name)).map(e => { return { key: e.key, label: e.name } });
        ListHelpers.exportList(firstRow, colNames, this.state.rows, sheetProps, fileName, (el) => el ? (el?.modifiedOn as Date) : "");
    }
