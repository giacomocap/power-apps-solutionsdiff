...
namespace ServiceFramework.DataAccess
{
    public static class DataMapper
    {
        ...
        {
            if (targetType == null || ob == null || ((dynamic)ob).IsNull)
            {
                return null;
            }
            var objVal = ((dynamic)ob).Value;
            if (objVal == null)
            {
                return null;
            }
            return JsonConvert.DeserializeObject(JsonConvert.SerializeObject(objVal), targetType);
        }
